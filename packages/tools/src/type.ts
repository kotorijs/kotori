 ...
 * @LastEditTime: 2023-09-10 15:53:25
 */
...
// export type Any = string | number | object | Any[] | boolean | Set<Any> | Map<Any, Any> | obj;

export interface ConnectMethod {
	send: FuncSend | null;
	listen: FuncListen | null;
}
...
export type PluginEntityClass = new (
	Event: Event,
	Api: Api,
	Const?: Const,
	Proxy?: object,
...
export interface Api {
	...
	set_friend_add_request: (file: string, remark: string, approve: boolean) => void;
	set_group_add_request: (file: string, type: 'add' | 'invite', reason: string | undefined, approve: boolean) => void;
	...
	send_group_notice: (group_id: number, content: string, image?: string) => void;
	get_group_notice: (group_id: number) => void;
	...
	download_file: (url: string, thread_count: number, headers: string | object[]) => void;
	check_url_safely: (url: string) => void;
	get_word_slices: (content: string) => void;
	handle_quick_operation: (context: object, operation: object) => void;
}
