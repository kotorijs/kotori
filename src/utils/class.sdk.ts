...
export class SDK {
     ...
     * @return {*}
     */
    public static cq_record = (target: string, magic: 0 | 1 = 0, cache: 0 | 1 = 1, proxy: 0 | 1 = 1, timeout?: number) => {
        const data: M.MessageRecord = target.includes('http') ? {
            file: target, url: target, cache, proxy, timeout
        } : {
            file: target, magic
        };
    ...
    public static cq_music_custom = (url: string, audio: string, title: string, content?: string, image?: string) => {
        const data: M.MessageMusicCustom = { type: 'custom', url, audio, title, content, image };
        return this.sdk_cq('music', data);
    }
     ...
     * @param {M.MessageImage['subType']} sub_type 图片子类型,只出现在群聊.
     ...
     * @return {*}
     */
    public static cq_image = (file: string, type: M.MessageImage['type'] = 'normal', sub_type?: M.MessageImage['subType'], url?: string, cache: M.MessageImage['cache'] = 1, id: M.MessageImage['id'] = 40000, c: M.MessageImage['c'] = 1) => {
        const data: M.MessageImage = { file, type, subType: sub_type, url, cache, id, c };
    ...
    public static cq_forward_node = (id: number, name?: string, uin?: number, content?: M.Message, seq?: M.Message) => {
        const data: M.MessageNode = typeof name === 'string' && typeof uin === 'number' && typeof content === 'object' && typeof seq === 'object' ? { name, uin, content, seq } : { id };
    ...
    public static cq_cardimage = (file: string, minwidth: number = 400, minheight: number = 400, maxwidth: number = 500, maxheight: number = 500, source: string = '', icon?: string) => {
        const data: M.MessageCardImage = { file, minwidth, minheight, maxwidth, maxheight, source, icon };
