...
export function main(ctx: Context) {
  ctx
    .command('motd')
    .action(async data => {
      const res = await ctx.http.get('https://api.hotaru.icu/api/motd', { ip: data.args[0], port: data.args[1] });
  ...
  ctx
    .command('motdbe')
    .action(async data => {
      const res = await ctx.http.get('https://api.hotaru.icu/api/motdpe', { ip: data.args[0], port: data.args[1] });
      if (!isObj(res)) return ['BOT_RESULT.SERVER_ERROR', { res }];
      if ((res.code !== 500 && typeof res.code === 'number') || !isObj(res.data)) {
        return [
