import { Context, MessageQuickReal, isObj } from 'kotori-bot';
import { resolve } from 'path';

export default function main(ctx: Context) {
	ctx.uselang(resolve(__dirname, '../locales'));

	ctx.command('hitokoto')
		.action(
			() =>
				'%HEAD%' +
				'\n一言 一言2' +
		...
		.help('hitokotos.help.hitokotos.help');
	...
	const hitokotoT = async (msg: number): Promise<MessageQuickReal> => {
		const res = await ctx.http.text('words', { msg, format: 'text' });
		if (!res || typeof res !== 'string') return 'Server error!Please contact ';
