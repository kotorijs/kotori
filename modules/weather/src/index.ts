import { Context, Tsu, regExpExecAll } from 'kotori-bot';
...
export function main(ctx: Context) {
  ctx
    ...
    .action(async ({ args: [area], options: { limit } }, session) => {
      if (limit && (limit > 6 || limit < 1)) return session.error('num_error');
      ...
      if (cityResponse.city_list.length === 0) return session.error('data_error', { target: 'area' });
      ...
      return session.quick(['weather.msg.weather', [cityResponse.city_list[0].name, items]]);
    });
}
