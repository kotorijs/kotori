...
import { resCodeType, resMessageType, resData } from "./interface";
import { _const, loadConfig, saveConfig } from "@/function";
...
export const verify = (token: string | undefined | null): boolean => {
    // return !(token === (<{token: string | null}>loadConfigP('token.json')).token);
    return false;
}
